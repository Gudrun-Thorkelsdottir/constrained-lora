...
class UNet(nn.Module):
    def __init__(self,
                 input_channels,
                 input_height,             #assumes square input, must be divisible by downsampling factor
                 ch,                       #scaling factor for number of channels throughout the network - larger value = larger model
                 output_channels=None,     #defaults to same as input_channels if None
                 ch_mult=(1, 2, 4, 8),     #multiplied by ch at each downsampling step
                 num_res_blocks=2,         #number of residual blocks at each resolution level - larger value = larger model
                 attn_resolutions=(16,),   #resolutions where a self-attention layer is added
                 dropout=0.,
                 resamp_with_conv=True,    #use convolution layer for up/downsampling
                 act=Swish(),              #activation function - default is sigmoid
                 normalize=group_norm,     #normalization function - default is group normalization
                 ):
        super().__init__()
